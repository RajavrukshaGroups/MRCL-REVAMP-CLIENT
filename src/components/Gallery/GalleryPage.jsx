...
//       category: 'lifestyle',
//       categoryLabel: 'Lifestyle',
//       image: ASSETS.ayanurLayoutAerial,
...
//       category: 'development',
//       categoryLabel: 'Development',
//       image: ASSETS.sylvanEventEntrance,
...
const LazyImage = ({ src, alt, className }) => {
  return (
    <img
      ...
      alt={alt}
      loading="lazy"
      className={`${className} bg-[#e8dec7]/20`}
      referrerPolicy="no-referrer"
    />
  );
};
...
export default function GalleryPage() {
  ...
  const [sugarlandLoaded, setSugarlandLoaded] = useState(false);
  const [sylvanLoaded, setSylvanLoaded] = useState(false);

  useEffect(() => {
    const preloadSection = async (items, setLoaded) => {
      try {
    ...
    preloadSection(sugarlandItems, setSugarlandLoaded);
    preloadSection(sylvanItems, setSylvanLoaded);
  }, []);
      ...
      <section className="py-16 px-4 sm:px-6 lg:px-8 space-y-20 max-w-7xl mx-auto">
        ...
        {!sugarlandLoaded ? (
          <div className="flex flex-col items-center justify-center min-h-[30vh] space-y-4">
            <div className="w-10 h-10 border-4 border-[#c5a85c]/30 border-t-[#c5a85c] rounded-full animate-spin"></div>
            <p className="font-serif text-[#b5802f] tracking-[0.2em] uppercase text-[10px] animate-pulse">Loading Sugarland Gallery...</p>
          </div>
        ) : (
        <div className="space-y-8">
        ...
        {!sylvanLoaded ? (
          <div className="flex flex-col items-center justify-center min-h-[30vh] space-y-4">
            <div className="w-10 h-10 border-4 border-[#c5a85c]/30 border-t-[#c5a85c] rounded-full animate-spin"></div>
            <p className="font-serif text-[#b5802f] tracking-[0.2em] uppercase text-[10px] animate-pulse">Loading Sylvan Woods Gallery...</p>
          </div>
        ) : (
        <div className="space-y-8">
